...
const EditDoctor = () => {
  ...
  useEffect(() => {
    if (doctors.length > 0) {
      const docData = doctors.find(doc => doc._id === id)
      if (docData) {
        setName(docData.name)
        setEmail(docData.email)
        setPrevImg(docData.image)
        setExperience(docData.experience)
        setFees(docData.fees)
        setAbout(docData.about)
        setSpeciality(docData.speciality)
        setDegree(docData.degree)
      ...
      <div className='bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll'>
        <div className='flex items-center gap-4 mb-8 text-gray-500'>
          <label htmlFor="doc-img">
            <img className='w-16 bg-gray-100 rounded-full cursor-pointer' src={docImg ? URL.createObjectURL(docImg) : prevImg || assets.upload_area} alt="" />
          </label>
        ...
        <div className='flex flex-col lg:flex-row items-start gap-10 text-gray-600'>
          ...
          <div className='w-full lg:flex-1 flex flex-col gap-4'>
            ...
            <div className='flex-1 flex flex-col gap-1'>
              <p>Doctor Password</p>
              <input onChange={(e) => setPassword(e.target.value)} value={password} className='border rounded px-3 py-2' type="password" placeholder='Leave blank to keep current' />
            </div>
